...
问题：一些进程可以提议值。一个共识算法保证这些值中只会有一个值被选定。
...
fail-stop 故障模型：服务器可能崩溃，或者可能会停止并重启
...
最简单的方法：只使用一个 Acceptor。所有的 Proposer 都将值发给某个 Acceptor，然后由它来从中选择一个值。缺点是如果 Acceptor 选定某个值后就宕机了，那么我们无法得知被选定的值是哪个，只能等 Acceptor 重启，这不满足算法的活性要求。**所以必须使用多个 Acceptor**。

使用多个 Acceptor，可能一个 Acceptor 接受的值是 v1，一个 Acceptor 接受的值是 v2，那么以谁的为准呢？**如果一个值 v 被多数派 Acceptor 接受了，那么它就被选定了**。通常使用奇数个 Acceptor，例如 3, 5, 7 等，如果使用偶数个 Acceptor，会存在一般赞同一般反对的情况。基于使用多个 Acceptor 的方式，即使有 Acceptor 宕机，只有还有多数派 Acceptor 存活，我们就能选定某个值并且能够知道选定的值是哪个。
...
假设一个编号为 m，值为 v 的提案被选定，任何编号大于 n > m 的提案的值都是 v。

我们对 n 的归纳来证明这个性质：如果编号 m..(n-1) 的提案的值都是 v，那么编号为 n 提案的值也为 v。

为了编号为 m 的提案能够被选定，那么一定存在一个集合 C，由多数派 Acceptor 组成，其中的每个 Acceptor 都接受了这个提案。

CP1. 如果一个提案 Proposal-j 最终会被通过，那么对于任意一个提案 Proposer-i，如果 Proposer-i.id > Proposer-j.id，那么必定 Proposer-i.v == Proposer-j.v

CP1_2. 如果一个提案 Proposal-j 最终会被通过，那么对于任意一个提案 Proposer-i，如果 Proposer-i.id > Proposer-j.id 且 proposal-id 在区间 [j, i) 内的任意提案的值均为 Proposer-j.v，那么必定 Proposer-i.v == Proposer-j.v

由 CP_2 可知，对于任意一个法定集合 Q，Q 最终会接受的所有 proposer-id 小于 i 的提案组成的集合 K，如果 K 中 proposer-id 最大的提案的值为 Proposer-j.v，那么 Proposer-i.v == Proposer-j.v
